...
class AsyncQueue {
	...
	constructor({ name, parallelism, parent, processor, getKey }) {
		...
		this.hooks = {
			...
			beforeAdd: new AsyncSeriesHook(["item"], "AsyncQueue.beforeAdd"),
			/** @type {SyncHook<[T]>} */
			added: new SyncHook(["item"], "AsyncQueue.added"),
			/** @type {AsyncSeriesHook<[T]>} */
			beforeStart: new AsyncSeriesHook(["item"], "AsyncQueue.beforeStart"),
			/** @type {SyncHook<[T]>} */
			started: new SyncHook(["item"], "AsyncQueue.started"),
			/** @type {SyncHook<[T, Error, R]>} */
			result: new SyncHook(["item", "error", "result"], "AsyncQueue.result")
		};
